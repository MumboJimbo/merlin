...
namespace Merlin.Profiles {
	...
	public class Combat {
        ...
        private static Player Player => GameManager.GetInstance().GetLocalPlayerCharacterView();
		...
		private void Idle() {
			...
			if (Player.GetCharacter().GetHealth() <= Player.GetCharacter().GetMaxHealth() * 0.5f) {
		...
		private void Attack(FightingObjectView target) {
            long attackTimer = new Duration(Player.GetAttackDelay()).GetMilliSeconds();
			...
			if (Player.GetCharacter().GetHealth() <= (Player.GetCharacter().GetMaxHealth() * 0.1f)) {
				SetState(State.Flee);
				return;
			}
		...
		private void Dodge(FightingObjectView target, Evade evade, bool shouldInterrupt = true) {
			...
			switch (evade) {
				case Evade.Behind:
					...
					Player.RequestMove(new Point2(movePos.x, movePos.y));
					evading = true;
		...
		private void Recover() {
			var recoverySpell = Player.GetSpells().Slot(CharacterSpellSlot.Armor).FirstOrDefault();
			...
			if (Player.GetCharacter().GetHealth() > Player.GetCharacter().GetMaxHealth() * 0.75f) {
				SetState(State.Idle);
			}
		}
		...
		protected virtual void Flee() {
			if (Player.GetCharacter().GetHealth() <= 0)
				SetState(State.Respawn);
			if (Player.GetCharacter().GetHealth() == Player.GetCharacter().GetMaxHealth())
				SetState(State.Idle);
		...
		private Spell[] GetUsableSpells() {
			...
			for (int i = spells.Length - 1; i > 0; i--) {
				...
				if (spells[i].SpellSlot == CharacterSpellSlot.Potion || spells[i].SpellSlot == CharacterSpellSlot.Food)
					continue;
		...
		private bool DefaultShouldDodge(FightingObjectView target, out Evade evade) {
			...
			var spellCategory = target.GetSpellCasted().d4.ToWrapped();
			var spellTarget = target.GetSpellCasted().d1.ToWrapped();
