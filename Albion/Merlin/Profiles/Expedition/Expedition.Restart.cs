	...
	partial class Expedition {
		...
		void Restart() {
			...
			if (!IsInExpedition())
				...
				if (_client.GetState() != GameState.Playing) return;
		...
		bool IsInExpedition() {
			if (_world.GetCurrentCluster().GetClusterNameInformation().ClusterType == Albion.Common.GameData.WorldInfos.ClusterTypes.Expedition)
				return true;
			return false;
		}
